...
Sphere::Sphere()
{
    ...
    //setupVertices();
    setupBuffers();
    setupModelMatrix(glm::vec3(0., 0., 0.), 0., 2.);
}
...
void Sphere::setupVertices() {
    ...
    std::cout << "Vertice size" << vert.size() << std::endl;
    std::cout << "Texture size" << tex.size() << std::endl;
    std::cout << "Normal size" << norm.size() << std::endl;
...
void Sphere::init(int prec) {
    ...
    for (int i = 0; i < numVertices; i++) { Vertices.push_back(Vertex(glm::vec3(), glm::vec3(), glm::vec2())); }
    ...
    for (int i = 0; i <= prec; i++) {
        for (int j = 0; j <= prec; j++) {
            ...
            Vertices[i * (prec + 1) + j] = Vertex(vertices[i * (prec + 1) + j], normals[i * (prec + 1) + j], texCoords[i * (prec + 1) + j]);
        }
    }
