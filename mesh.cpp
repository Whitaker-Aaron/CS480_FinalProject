...
Mesh::Mesh()
{
	...
	// Model Set Up
	angle = 0.0f;
	pivotLocation = glm::vec3(0.f, 0.f, 20.f);
...
Mesh::Mesh(glm::vec3 pivot, const char* fname, const char* tname, bool instanced)
{
	// Vertex Set Up
	loadModelFromFile(fname);
...
void Mesh::Render(GLint posAttribLoc, GLint colAttribLoc)
{
	...
	if(instanced_ == false){
		...
		glDrawElementsInstanced(GL_TRIANGLES, Indices.size(), GL_UNSIGNED_INT, 0, 20);
	}
...
bool Mesh::loadModelFromFile(const char* path) {
	...
	for (int i = 0; i < scene->mNumMeshes; i++) {
		...
		for (int j = 0; j < iMeshFaces; j++) {
			...
			for (int k = 0; k < 3; k++) {
				Vertices.push_back(Vertex(glm::vec3(mesh->mVertices[face.mIndices[k]].x, mesh->mVertices[face.mIndices[k]].y,
					mesh->mVertices[face.mIndices[k]].z), mesh->HasNormals() ? glm::vec3(mesh->mNormals[face.mIndices[k]].x,
						mesh->mNormals[face.mIndices[k]].y, mesh->mNormals[face.mIndices[k]].z) : glm::vec3(1.f, 1.f, 1.f), glm::vec2(mesh->mTextureCoords[0][face.mIndices[k]].x,
